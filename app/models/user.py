...
class User(Base):
    __tablename__ = 'users'
    id = sa.Column(sa.UUID(as_uuid=True),primary_key=True,default=uuid.uuid4,unique=True)    
    ...
    products:so.Mapped[List["Product"]] = so.relationship(back_populates='user',cascade="all,delete-orphan")
