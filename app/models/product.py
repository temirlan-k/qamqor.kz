...
class Product(Base):
    ...
    created_at:so.Mapped[datetime.datetime] = so.mapped_column(server_default=sa.func.now(),default=datetime.datetime.now)
    update_time: so.Mapped[datetime.datetime] = so.mapped_column(server_default=sa.func.now(),server_onupdate=sa.func.now())
    ...
    orders: so.Mapped[List['Order']] = so.relationship(secondary=order_product_association_table,back_populates='products')
